...
class RunConfig:
    ...
    shard_index: int = 0
    shard_count: int = 1
    prompt_cache_namespace: str = "default"
...
def _units(groups: list[ImageGroup], prompts: dict[str, Any], batch_size: int):
    ...
    for group in groups:
        ...
        for chunk in chunks:
            unit = InferenceUnit(group.image_key, group.image_path, tuple(chunk),
                                 tuple(prompts[r.query_id].text for r in chunk))
...
def _split(unit: InferenceUnit) -> tuple[InferenceUnit, ...]:
    ...
    return tuple(
        InferenceUnit(unit.image_key, unit.image_path, tuple(unit.records[start:end]), tuple(unit.prompts[start:end]))
        for start, end in ((0, middle), (middle, len(unit.records))) if start < end)


def _infer_resilient(backend: GroundingBackend, units: list[InferenceUnit]) -> list[
    tuple[InferenceUnit, tuple[str, ...], dict[str, Any] | None]]:
    try:
        ...
        logging.getLogger("grounding").warning("显存不足：当前工作量 %d，后续 batch 降至不超过 %d", total,
                                               backend._recommended_batch_size)
...
def run(config: RunConfig, provider: PromptProvider, backend: GroundingBackend) -> dict[str, Any]:
    ...
    groups, queries = load_query_groups(config.data_dir, config.query_file, image_field=config.image_field,
                                        limit=config.limit)
    prompt_started = time.time()
    LOG.info(
        "prompt build start queries=%d provider=%s version=%s",
        len(queries), provider.name, provider.version,
    )
    prompt_results = prepare_prompt_results(
        groups,
        provider,
        PromptCache(config.prompt_cache, namespace=config.prompt_cache_namespace),
    )
    LOG.info(
        "prompt build complete queries=%d elapsed=%.2fs",
        len(prompt_results), time.time() - prompt_started,
    )
    ...
    pending_groups = [ImageGroup(group.image_key, group.image_path,
                                 tuple(record for record in group.records if record.query_id not in results)) for group
                      in groups]
    pending_groups = [group for group in pending_groups if group.records]
    processed = 0
    effective_batch_size = config.batch_size
    LOG.info(
        "run start groups=%d queries=%d pending=%d resumed=%d batch_size=%d cache_namespace=%s",
        len(groups),
        len(queries),
        sum(len(group.records) for group in pending_groups),
        len(results),
    ...
    while group_index < len(pending_groups):
        ...
        while group_index < len(pending_groups) and batch_rows < effective_batch_size:
            ...
            batch.append(InferenceUnit(group.image_key, group.image_path, chunk,
                                       tuple(prompt_results[r.query_id].text for r in chunk)))
        ...
        LOG.info(
            "batch start rows=%d units=%d completed=%d/%d batch_size=%d",
            batch_rows,
            len(batch),
            len(results),
            len(queries),
        ...
        try:
            ...
            for unit in batch:
                for record in unit.records:
                    ...
                    append_jsonl(config.raw,
                                 {"query_id": record.query_id, "image_key": record.image_key, "status": "error",
                                  "raw_output": "", "bbox": None, "parse_status": "error", "error": message,
                                  "prompt_provider": prompt.provider, "prompt_version": prompt.version,
                                  "prompt_profile": getattr(provider, "profile", None), "prompt": prompt.text})
        ...
        for unit, texts, stats in rows:
            for record, raw_text in zip(unit.records, texts):
                ...
                prompt = prompt_results[record.query_id]
                append_jsonl(config.raw, {"query_id": record.query_id, "image_key": record.image_key,
                                          "status": "success" if parsed else "fallback", "raw_output": raw_text,
                                          "bbox": bbox, "parse_status": "ok" if parsed else "fallback", "error": None,
                                          "prompt_provider": prompt.provider, "prompt_version": prompt.version,
                                          "prompt": prompt.text,
                                          "prompt_sha256": hashlib.sha256(prompt.text.encode("utf-8")).hexdigest(),
                                          "prompt_profile": getattr(provider, "profile", None),
                                          "runtime_stats": stats,
                                          "elapsed_seconds": round(time.time() - batch_started, 6)})
                processed += 1
        ...
        atomic_dump(results, config.partial)
        LOG.info(
            "batch complete rows=%d elapsed=%.2fs completed=%d/%d next_batch_size=%d",
            batch_rows,
    ...
    config.partial.unlink(missing_ok=True)
    return {"count": len(results), "new": processed, "skipped": len(results) - processed, "output": str(config.output),
