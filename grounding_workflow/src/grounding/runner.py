...
def run(config: RunConfig, provider: PromptProvider, backend: GroundingBackend) -> dict[str, Any]:
    ...
    results = load_json_object(config.partial) if config.resume else {}
    ...
    processed = 0
    for batch in _units(pending_groups, prompt_results, config.batch_size):
        ...
        for unit, texts, stats in rows:
            for record, raw_text in zip(unit.records, texts):
                ...
                append_jsonl(config.raw, {
                    ...
                    "raw_output": raw_text,
                    "bbox": bbox,
                    "parse_status": "ok" if parsed else "fallback",
                    "error": None,
                    "prompt_provider": prompt.provider,
                    "prompt_version": prompt.version,
                    "prompt": prompt.text,
                ...
                processed += 1
        # 每个 batch 都落盘；save_every 只控制日志频率，不影响可恢复性。
        atomic_dump(results, config.partial)
        if processed and (processed % config.save_every < len(batch) or len(results) == len(queries)):
            LOG.info("断点保存: %d/%d", len(results), len(queries))
