...
from grounding.backends import InternVLBackend, InternVLConfig, LocateAnythingBackend, LocateAnythingConfig, \
    QwenBackend, QwenConfig
from grounding.prompts import PROMPT_PROFILES, default_prompt_profile, load_provider
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prompt_profile", choices=PROMPT_PROFILES, default=None)
    ...
    parser.add_argument("--scheduler", choices=("eager", "hold_ar", "ar_first", "pipeline", "adaptive"),
                        default="eager")
...
def main() -> int:
    ...
    log_file = (args.log_file or output.with_suffix(".log")).resolve()
    ...
    logging.info(
        "startup backend=%s model=%s batch_size=%d visible_gpus=%s log=%s",
        args.backend,
        args.model,
        args.batch_size,
    ...
    prompt_profile = args.prompt_profile or default_prompt_profile(args.backend)
    provider = load_provider(args.prompt_provider, prompt_profile)
    logging.info(
        "prompt provider=%s version=%s profile=%s",
        provider.name,
        provider.version,
    ...
    if args.backend == "qwen":
        backend = QwenBackend(
            QwenConfig(args.model, args.dtype, args.max_new_tokens or 128, args.min_pixels, args.max_pixels,
                       args.attn_implementation, args.gpu_memory_limit))
    elif args.backend == "locateanything":
        backend = LocateAnythingBackend(
            LocateAnythingConfig(args.model, args.attn, args.vision_attn, args.scheduler, args.group_size,
                                 args.max_new_tokens or 2048, args.temperature, args.top_p, args.top_k,
                                 args.repetition_penalty, args.feature_cache_size))
    else:
        if args.internvl_image_cache_size < 0:
            raise SystemExit("--internvl_image_cache_size 不能为负数")
        backend = InternVLBackend(
            InternVLConfig(args.model, args.internvl_dtype, args.max_new_tokens or 128, args.internvl_image_cache_size,
                           args.gpu_memory_limit))
    prompt_namespace = f"{provider.name}:{provider.version}"
    config = RunConfig(
        args.data_dir.resolve(),
        args.query_file.resolve() if args.query_file else None,
        output,
        (args.partial or output.with_suffix(".partial.json")).resolve(),
        (args.raw_output or output.with_suffix(".raw.jsonl")).resolve(),
        (args.prompt_cache or output.with_suffix(".prompts.jsonl")).resolve(),
        args.image_field,
        args.limit,
        args.batch_size,
        args.save_every,
